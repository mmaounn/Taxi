...
import { FleetSummaryPDF } from "@/lib/pdf/fleet-summary-pdf";
...
export async function GET(req: NextRequest) {
  ...
  const totals = rows.reduce(
    ...
    { totalPlatformNet: 0, partnerCommission: 0, vehicleRental: 0, lineItemsTotal: 0, driverNetEarnings: 0, cashCollected: 0, payoutAmount: 0 }
  );

  const pdfElement = React.createElement(FleetSummaryPDF, {
  ...
  const filename = `fleet_summary_${periodStart}_${periodEnd}.pdf`;
